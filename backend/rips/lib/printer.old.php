...
**/
	...
	{	
									...
									} else
									{
										foreach($key as $token)
										{
											if(is_array($token))
											{
												...
												//$text .= htmlentities($token, ENT_QUOTES, 'utf-8');
												$text .= $token;
										}
									}
									//$text .= '<span class="phps-code">]</span>';
		...
		return str_replace('&lt;br/&gt;', '<br/>', htmlentities($output, ENT_QUOTES, 'utf-8')) . "', ";
	}
	...
	function traverseBottomUp($tree) 
	{
		...
		switch($tree->marker) 
		{
			case 1: break; //echo ' class="userinput"'; break;
			case 2: break; //echo ' class="validated"'; break;
			case 3: break; //echo ' class="functioninput"'; break;
			case 4: break; //echo ' class="persistent"'; break;
		}
		
		//echo '><li>' . $tree->value;

		echo $tree->value, "<br/>"; //nishant
	...
	function dependenciesTraverse($tree) 
	{
		if(!empty($tree->dependencies))
		{
			...
			foreach ($tree->dependencies as $linenr=>$dependency) 
			{
				if(!empty($dependency))
				{
					...
					$buffer .= "{" . highlightline($dependency, '', $linenr) . '},'; //nishant
		...
		$buffer = str_replace(",}", "}", str_replace(",]", "]", str_replace(", }", "}", $buffer)));
		echo $buffer;
	}
				...
				{		
					...
					{	
						if($vulnBlock->vuln)	
						{
							...
							echo "'warning_type': '" . $vulnBlock->category."', "; //nishant
							...
							foreach($vulnBlock->treenodes as $tree)
							{
								...
								{	
									...
									*/
									...
									echo "'description': '" . $tree->title . "', "; //nishant
							...
							if(!empty($vulnBlock->alternatives))
							{
								...
								echo 'Vulnerability is also triggered in:</br>';
